        ...
        try:
            ...
            for tf in range(in_f, out_f):
                ...
                if fg is not None and mt is not None:
                    ...
                    if _write_combined:
                        # 2026-05-14: per-mask dispatch with chroma-aware dilate_into.
                        # Each SAM mask gets its own apply_sam2_gate_subtract call so
                        # EDGE GUARD dilation only extends into pixels that match the
                        # mask's role. MASK 1 (body) extends into low-alpha (hair tips,
                        # residual green) NOT into solid background — kills the "hair
                        # blob" halo. MASK 2 (feet) extends into high-alpha (the foot
                        # itself) NOT into background — kills the 10-px foot slop.
                        # Per-mask alphas unioned via union_alpha. Pattern matches the
                        # viewer's _combine_one_mask path (subtract mode + dilate_into).
                        _probe(f"BEFORE_COMBINED_MERGE_{tf}")
                        if _gates_for_sam:
                            try:
                                from sam2_combine import (
                                    apply_sam2_gate_subtract as _agsr,
                                    union_alpha as _ualpha,
                                )
                                _buf = int(settings.get("sam2_margin", 20) or 20)
                                _fpx = max(_buf // 2, 1)
                                _per_alphas = []
                                for _oid, _gate in zip(_obj_ids, _gates_for_sam):
                                    _di = "low_alpha" if _oid == 1 else "high_alpha"
                                    _pa = _agsr(
                                        _m, _gate, fr,
                                        buffer_px=_buf,
                                        feather_px=_fpx,
                                    ...
                                    _per_alphas.append(_pa)
                                _m_combined = _ualpha(*_per_alphas)
                                if _m_combined is None:
                                    _m_combined = _m
                            except Exception as _mge:
