...
def get_settings():
    return {
        ...
        # margin/soften sliders moved to viewer; non-zero defaults match the
        # 2-week-stable behavior (panel sliders defaulted to Value=10/10=1.0).
        # Yesterday's slider migration silently zeroed these → SAM2 gate ran
        # tight, leaving leg-leak / foot-edge garbage on SCRUB.
        "sam2_margin": float(SAM2_MATTE_MARGIN),
        "sam2_soften": 1.0,
    }
    ...
    try:
        ...
        if mt is not None and _s2_masks and frame_idx in _s2_masks:
            ...
            if _gate.shape != mt.shape:
                # INTER_LINEAR — the 2-week-stable behavior. INTER_AREA + 3px
                # Gaussian (tried 2026-04-27 morning) softened the gate edge
                # enough to leak floor through the gap-between-legs and behind
                # props, producing "garbage mattes" on RANGE renders.
