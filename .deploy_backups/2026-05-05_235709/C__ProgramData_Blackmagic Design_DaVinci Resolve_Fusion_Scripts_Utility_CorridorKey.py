...
def _panel_dispatch_sam2_combine(alpha, gates, src_rgb, settings, obj_ids=None):
    if not gates:
        return alpha
    import cv2, numpy as np
    from sam2_combine import apply_sam2_junk_kill
    # JUNK KILLER (panel render parity with viewer). Industry-standard
    # garbage matte combine: alpha = NN * (1 - junk_zone) where
    # junk_zone = NOT dilate(union SAM2) AND NOT chroma_screen.
    # Per-mask SAFETY radius: EDGE GUARD for MASK 1, FEET GUARD for MASK 2.
    # Hole-close per mask before dilation fills small SAM2 interior gaps.
    ...
    for i, g in enumerate(gates):
        ...
        if (oid == 1 and _m1b) or (oid == 2 and _m2b):
            continue
        if g.shape != alpha.shape:
            g = cv2.resize(g, (alpha.shape[1], alpha.shape[0]),
                           interpolation=cv2.INTER_LINEAR)
        bin_g = (g > 0.5).astype(np.uint8)
        close_kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (51, 51))
        bin_g = cv2.morphologyEx(bin_g, cv2.MORPH_CLOSE, close_kernel)
        if n_active > 1 and oid is not None:
            pad = edge_guard_user if oid == 1 else feet_guard_user
        else:
            pad = edge_guard_user
        if pad > 0:
            dk = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (2 * pad + 1, 2 * pad + 1))
            bin_g = cv2.dilate(bin_g, dk)
        trust_zones.append(bin_g)
    if not trust_zones:
        return alpha
    union_trust = trust_zones[0]
    for tz in trust_zones[1:]:
        union_trust = np.maximum(union_trust, tz)
    out = apply_sam2_junk_kill(
        alpha, union_trust.astype(np.float32),
        source_rgb=src_rgb,
        actor_pad_px=0,
        junk_feather_px=4,
        hole_close_px=0,
