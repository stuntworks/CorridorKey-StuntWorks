...
def reprocess_with_cached():
    ...
    try:
        ...
        # Last modified: 2026-05-13 | Change: Live re-key intentionally keeps CK alone (no SAM)
        # WHAT IT DOES: Skips SAM application. Live preview matte is CK only so the user can spot
        #               missing hair, evaluate SAM dot placement, and see CK's soft edges before
        #               SAM clamps them. The Combined tab in the viewer still composites CK x SAM
        #               for preview. SAM is applied at file-export time in process_current_frame.
        # DEPENDS-ON: nothing here. Intentional no-op for the slider re-key path.
        # DANGER ZONE: do NOT mirror process_current_frame's SAM multiply into this function.
        #              Berto needs CK-alone visibility in the live preview to evaluate keying quality.
...
def process_current_frame(preview_only=False):
    ...
    try:
        ...
        # Last modified: 2026-05-13 | Change: Restore SAM2 gate to single-frame keying (reverses 4314c6b removal)
        # WHAT IT DOES: Multiplies CK alpha by the SAM2 garbage matte loaded from session PNGs.
        #               Produces the combined CK x SAM mask the viewer's Combined tab shows.
        # DEPENDS-ON: _load_sam2_output_gate (line 1064), settings["alpha_method"], session SAM PNGs.
        # AFFECTS: mt (the alpha that save_output writes). When SAM is inactive (no dots, alpha_method != 1,
        #          or PNGs missing) _load_sam2_output_gate returns None and this block is a no-op,
        #          preserving the original "clean CK only" behavior.
        # DANGER ZONE: parallel logic lives in render_composite + _render_now matte branch.
        #              If you change the math here, also update preview_viewer_v2.py:2751 (Combined view).
        if mt is not None:
