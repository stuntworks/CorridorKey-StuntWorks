...
def _panel_dispatch_sam2_combine(alpha, gates, src_rgb, settings, obj_ids=None):
    ...
    for i, g in enumerate(gates):
        ...
        if oid is not None and n_active > 1:
            local = dict(settings)
            # MASK 1 owns HALO BODY + EDGE GUARD + dilate_into=low_alpha
            # (body / on-green parts — extends into hair / green only).
            # MASK 2 owns HALO FEET + FEET GUARD + dilate_into=high_alpha
            # (off-green parts — extends into wall/floor only).
            ...
            local["_dilate_into"] = "low_alpha" if oid == 1 else "high_alpha"
        else:
            local = settings
