        ...
        try:
            ...
            for tf in range(in_f, out_f):
                ...
                if fg is not None and mt is not None:
                    ...
                    if _write_combined:
                        # 2026-05-14 (corrected): simple multiply matches viewer's
                        # Combined preview math at preview_viewer_v2.py:2751:
                        #   _combo = np.clip(alpha * _sam_arr, 0.0, 1.0)
                        # Berto's [[corridorkey_subtract_is_simple_multiply]] memory:
                        # "NN_alpha * dilate(SAM2_silhouette, margin_px) with a
                        #  Gaussian-feathered boundary. That's it."
                        # The dilate + Gaussian feather is already baked into
                        # _sam_matte_v1 by process_sam_matte() above (margin_px,
                        # softness_sigma, fill_kernel_px). So all this branch does
                        # is multiply CK alpha by the processed SAM matte. No
                        # per-mask dispatch, no dilate_into, no apply_sam2_gate_subtract.
                        _probe(f"BEFORE_COMBINED_MERGE_{tf}")
                        if _sam_matte_v1 is not None:
                            try:
                                _sm_2d = _sam_matte_v1[:, :, 0] if _sam_matte_v1.ndim == 3 else _sam_matte_v1
                                if _sm_2d.shape != _m.shape:
                                    _sm_2d = cv2.resize(_sm_2d, (_m.shape[1], _m.shape[0]), interpolation=cv2.INTER_LINEAR)
                                _m_combined = np.clip(_m * _sm_2d.astype(np.float32), 0.0, 1.0).astype(np.float32)
                            except Exception as _mge:
