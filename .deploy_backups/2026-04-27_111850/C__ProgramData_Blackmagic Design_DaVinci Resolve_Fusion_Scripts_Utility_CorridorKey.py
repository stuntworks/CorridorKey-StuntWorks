    ...
    try:
        ...
        with torch.inference_mode():
            ...
            for frame_idx, _obj_ids, mask_logits in predictor.propagate_in_video(state, start_frame_idx=earliest_click_temp):
                # BINARY mask: (logits > 0). Soft saturation ramp leaked partial
                # transparency into thin non-green regions like the gap between
                # legs or behind props, producing "garbage mattes" on RANGE renders.
                # Anti-aliasing happens downstream via INTER_AREA + 3px Gaussian
                # in the keying loop — that's the right place for edge softness.
