    ...
    if settings.get("alpha_method") == 1:
        ...
        if _pos or _neg:
            ...
            try:
                ...
                if _n_good > 0:
                    # MULTI-FRAME CLICK CHECK (2026-04-27): SCRUB samples are
                    # non-contiguous — sampled_tl_frames is a sparse list of
                    # timeline frames, not [in_f..out_f). Per-click frame anchoring
                    # requires every click frame to land on a sampled frame OR be
                    # added to the temp dir. V1 keeps it simple: if all clicks are
                    # on a single sampled frame, use single-anchor (legacy). If
                    # clicks span multiple frames OR a click is at a non-sampled
                    # frame, refuse with a clear message and skip propagation —
                    # the user can clear and re-click on a single sampled frame.
                    # Full multi-frame is enabled for PROCESS RANGE below.
                    _click_frames = sorted({int(c["frame"]) for c in sam_clicks if c.get("frame") is not None})
                    _good_tl_set = {sampled_tl_frames[i] for i in _good_orig_idxs} if sampled_tl_frames else set()
                    _bad_clicks = [f for f in _click_frames if f not in _good_tl_set]
                    if len(_click_frames) > 1 or _bad_clicks:
                        log(f"SCRUB: cannot propagate — click frames {_click_frames} include "
                            f"out-of-scrub or multiple frames. Bad: {_bad_clicks}. "
                            "Clear dots and re-click on a single frame INSIDE the scrub range.")
                        status("SCRUB: click on a single frame inside the range, then run scrub.")
                        scrub_sam2_masks = {}
                    else:
