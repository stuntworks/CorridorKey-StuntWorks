...
def _panel_dispatch_sam2_combine(alpha, gates, src_rgb, settings, obj_ids=None):
    ...
    for i, g in enumerate(gates):
        ...
        if oid is not None and n_active > 1:
            ...
            # generous margin recovers hair / butt). MASK 2 owns HALO FEET +
            # FEET GUARD (off-green parts — tight margin avoids pillow into
            # wall/floor). Halos and margins zero/swap so each mask only
            # acts in its own domain.
