# Last modified: 2026-04-26 | Change: show_preview_window also writes original.png (raw source frame) so the viewer's Original view mode shows the unprocessed footage instead of the NN-clean FG | Full history: git log
...
# WHAT IT DOES: Per-click frame tracking — each click stores its own frame number.
#   Lets SAM2 video propagation place each click on its correct frame instead of
#   forcing all clicks onto a single anchor frame (which broke whenever the user
#   moved the playhead between an APPLY MASK and a refining click).
#   Format: list of {"x": int, "y": int, "label": 0 or 1, "frame": int or None}.
#   Built from live_params.json sam_clicks by _merge_live_params; consumed by the
#   3 propagation call sites in on_scrub_range / BRAW PROCESS RANGE / regular
#   PROCESS RANGE. Falls back to sam_points if sam_clicks is empty (back-compat
#   with old live_params.json files).
# DEPENDS-ON: viewer's _save_live_params_now writing sam_clicks
# AFFECTS: run_sam2_video_propagation gets per-frame click groupings
sam_clicks = []
...
def _merge_live_params(settings):
    try:
        ...
        # NEW: per-click frame tracking. If sam_clicks is present, use it as the
        # source of truth and synthesize legacy sam_points from it for back-compat.
        # If only legacy fields exist (older viewer wrote them), fall back.
        global sam_clicks
        if "sam_clicks" in lp:
            sam_clicks = list(lp.get("sam_clicks") or [])
            sam_points["positive"] = [(int(c["x"]), int(c["y"])) for c in sam_clicks if c.get("label") == 1]
            sam_points["negative"] = [(int(c["x"]), int(c["y"])) for c in sam_clicks if c.get("label") == 0]
            # Anchor frame: first click's frame (matches the legacy fallback in the viewer).
            _first_frame = next((c.get("frame") for c in sam_clicks if c.get("frame") is not None), None)
            sam_points["frame"] = _first_frame if _first_frame is not None else lp.get("sam_anchor_frame")
            if lp.get("alpha_method") == 1 or sam_points["positive"]:
                out["alpha_method"] = 1
        elif "sam_positive" in lp or "sam_negative" in lp:
            # Legacy path — old live_params.json without sam_clicks. Synthesize a
            # sam_clicks list with all points anchored to one frame (the old global
            # anchor) so propagation code only has to handle one format.
            ...
            _legacy_frame = sam_points["frame"]
            sam_clicks = (
                [{"x": int(p[0]), "y": int(p[1]), "label": 1, "frame": _legacy_frame} for p in sam_points["positive"]]
                + [{"x": int(p[0]), "y": int(p[1]), "label": 0, "frame": _legacy_frame} for p in sam_points["negative"]]
            )
...
def generate_alpha_hint(frame, settings):
    # ALIGNED WITH AE 2026-04-27: switched from HSV-based AlphaHintGenerator
    # to AE's simple RGB chroma test.
    # Reason (Berto's stunt plate): the actor wore a tan MOLLE plate carrier.
    # NO green spill — actor was far enough from the green. But HSV thresholds
    # [35-85, 50-255, 50-255] cover hue range 35-85, which is yellow-green-cyan.
    # Tan / khaki / olive have hues in 25-55 — overlapping the lower bound.
    # The HSV detector was reading the vest itself as "screen color," kicking
    # those pixels out of the alpha hint. NN got "vest = background" as input
    # and produced gray inside the vest. AE's RGB test requires green to
    # genuinely dominate red AND blue by 20% — tan has red >= green, so it's
    # never flagged regardless of hue. Same fix, different mechanism than
    # green-spill (which was my first wrong guess).
    # To revert: `from core.alpha_hint_generator import AlphaHintGenerator;
    # return AlphaHintGenerator(screen_type=settings["screen_type"]).generate_hint(frame)`
    import numpy as np, cv2
    rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB).astype(np.float32) / 255.0
    if settings["screen_type"] == "green":
        green = rgb[:, :, 1]; red = rgb[:, :, 0]; blue = rgb[:, :, 2]
        screen_mask = (green > 0.3) & (green > red * 1.2) & (green > blue * 1.2)
    else:
        blue = rgb[:, :, 2]; red = rgb[:, :, 0]; green = rgb[:, :, 1]
...
def run_sam2_video_propagation(fp, ss, cs, in_f, out_f, pos_pts, neg_pts, anchor_frame_abs, click_groups=None):
    """Run SAM2 video propagation across a frame range.

    Two contracts:
      LEGACY (single-anchor): pos_pts, neg_pts, and anchor_frame_abs together
        describe one set of clicks placed on a single anchor frame. If the
        anchor falls outside [in_f, out_f), it falls back to anchor_rel=0,
        which silently mis-segments whatever happens to be at the click coords
        on the closest in-range frame. This was the dormant bug.
      MULTI-FRAME (preferred, click_groups != None): each click stores its
        own frame number. Click frames OUTSIDE [in_f, out_f) are still
        included in the propagation set so SAM2 can anchor correctly; their
        masks are dropped from the return.

    click_groups format: list of dicts:
        [{"x": int, "y": int, "label": 0|1, "frame": int_abs_frame_or_None}, ...]
        Entries with frame=None are dropped (we have no anchor for them).

    Returns: dict {range_relative_index: float32_mask} for indices in [0, out_f-in_f).
    """
    ...
    # Build the unified click_groups regardless of which contract the caller
    # used. If caller provided click_groups, use it as-is. Else synthesize one
    # from legacy pos_pts/neg_pts/anchor_frame_abs.
    if click_groups is None:
        click_groups = (
            [{"x": int(p[0]), "y": int(p[1]), "label": 1, "frame": anchor_frame_abs} for p in (pos_pts or [])]
            + [{"x": int(p[0]), "y": int(p[1]), "label": 0, "frame": anchor_frame_abs} for p in (neg_pts or [])]
        )

    # Determine the union of frames we need in the temp dir = scrub range +
    # any click frames that fall OUTSIDE the scrub range. Sort by absolute
    # frame number so SAM2's frame_idx matches timeline order.
    scrub_abs_frames = list(range(in_f, out_f))
    extra_click_frames_abs = sorted({
        int(c["frame"]) for c in click_groups
        if c.get("frame") is not None and not (in_f <= int(c["frame"]) < out_f)
    })
    all_abs_frames = sorted(set(scrub_abs_frames) | set(extra_click_frames_abs))
    if not all_abs_frames:
        log("SAM2 video: no frames to process")
        return {}
    abs_to_temp = {abs_f: i for i, abs_f in enumerate(all_abs_frames)}

    # Group clicks by absolute frame, keeping only those we can anchor.
    clicks_by_abs = {}
    for c in click_groups:
        af = c.get("frame")
        if af is None:
            continue
        af = int(af)
        if af not in abs_to_temp:
            log(f"SAM2 video: dropping click on frame {af} — not in temp set (this should not happen)")
            continue
        d = clicks_by_abs.setdefault(af, {"positive": [], "negative": []})
        if int(c.get("label", 1)) == 1:
            d["positive"].append((int(c["x"]), int(c["y"])))
        else:
            d["negative"].append((int(c["x"]), int(c["y"])))
    if not clicks_by_abs:
        log("SAM2 video: no valid clicks (all had frame=None)")
        return {}

    log(f"SAM2 video: scrub_dur={dur} extras={len(extra_click_frames_abs)} click_frames={sorted(clicks_by_abs.keys())}")

    tmp_dir = Path(tempfile.mkdtemp(prefix="ck_sam2_frames_"))
    try:
        # --- Export every frame in all_abs_frames to tmp_dir, in sorted order ---
        log(f"SAM2 video: exporting {len(all_abs_frames)} frames to {tmp_dir} ...")
        status(f"SAM2: exporting {len(all_abs_frames)} frames...")
        ...
        for temp_idx, abs_f in enumerate(all_abs_frames):
            if _tif_files:
                fidx = abs_f  # BRAW: tif file index = absolute frame number (in_f=0 for BRAW)
                frame = cv2.imread(str(_tif_files[fidx])) if 0 <= fidx < len(_tif_files) else None
                if frame is None:
                    log(f"SAM2 video: skipped unreadable BRAW frame {abs_f}")
                    continue
            else:
                sf = ss + (abs_f - cs)  # absolute_frame → source seek position
                _cap.set(cv2.CAP_PROP_POS_FRAMES, sf)
                ret, frame = _cap.read()
                if not ret:
                    log(f"SAM2 video: skipped unreadable frame {abs_f}")
                    continue
            cv2.imwrite(str(tmp_dir / f"{temp_idx:06d}.jpg"), frame,
                        [cv2.IMWRITE_JPEG_QUALITY, 95])
        if _cap:
            _cap.release()

        # --- Load video predictor ---
        log(f"SAM2 video: loading predictor, click_frames(temp_idx)={[abs_to_temp[a] for a in sorted(clicks_by_abs.keys())]} ...")
        status("SAM2: loading video model...")
        from sam2.build_sam import build_sam2_video_predictor
        predictor = build_sam2_video_predictor(cfg, ckpt, device=device)

        # earliest_click_temp_idx = SAM2's natural anchor for two-pass propagation.
        # SAM2 video predictor's propagate_in_video starts from the earliest frame
        # that has user input by default; passing start_frame_idx makes that explicit.
        earliest_click_abs = min(clicks_by_abs.keys())
        earliest_click_temp = abs_to_temp[earliest_click_abs]
        ...
        with torch.inference_mode():
            ...
            # MULTI-FRAME ADD: place clicks on each click frame separately.
            # clear_old_points=False on subsequent groups so we accumulate state
            # rather than wipe it. SAM2 supports this idiom natively.
            for i, abs_f in enumerate(sorted(clicks_by_abs.keys())):
                grp = clicks_by_abs[abs_f]
                tidx = abs_to_temp[abs_f]
                pts = grp["positive"] + grp["negative"]
                labs = [1] * len(grp["positive"]) + [0] * len(grp["negative"])
                if not pts:
                    continue
                predictor.add_new_points_or_box(
                    ...
                    clear_old_points=(i == 0),  # wipe on first call only; accumulate after
                )
                log(f"SAM2 video: clicks placed on temp_idx={tidx} (abs={abs_f}) — {len(grp['positive'])}+ {len(grp['negative'])}-")

            # --- Forward pass: earliest_click_temp → end ---
            ...
            log(f"SAM2 video: forward pass (start={earliest_click_temp} → temp_end {len(all_abs_frames)-1})")
            forward_count = 0
            for frame_idx, _obj_ids, mask_logits in predictor.propagate_in_video(state, start_frame_idx=earliest_click_temp):
                # SOFT MASK via saturation ramp on logits (same as single-frame
                # _apply_sam_mask). Binary thresholding (mask_logits > 0) caused
                # staircase artifacts after the gate is resized to alpha shape.
                # The ramp gives ~2-4px anti-aliased silhouette while still
                # saturating the confident interior to 1.0.
                _lg = mask_logits[0].squeeze().cpu().numpy().astype(np.float32)
                mask = np.clip(0.5 + _lg * 0.25, 0.0, 1.0)
                masks_by_temp[frame_idx] = mask
                forward_count += 1
                if frame_idx % 20 == 0:
                    log(f"SAM2 forward: temp_idx {frame_idx}/{len(all_abs_frames)}")
                    status(f"SAM2 forward: {frame_idx}/{len(all_abs_frames)} frames")
            log(f"SAM2 forward pass done — {forward_count} masks")

            # --- Backward pass: earliest_click_temp → 0 ---
            # Only needed when the earliest click isn't already at temp_idx 0.
            if earliest_click_temp > 0:
                status("SAM2: backward pass...")
                log(f"SAM2 video: backward pass (start={earliest_click_temp} → temp_idx 0)")
                backward_count = 0
                for frame_idx, _obj_ids, mask_logits in predictor.propagate_in_video(
                        state, start_frame_idx=earliest_click_temp, reverse=True):
                    if frame_idx not in masks_by_temp:
                        _lg = mask_logits[0].squeeze().cpu().numpy().astype(np.float32)
                        mask = np.clip(0.5 + _lg * 0.25, 0.0, 1.0)
                        masks_by_temp[frame_idx] = mask
                    backward_count += 1
                    if frame_idx % 20 == 0:
                        log(f"SAM2 backward: temp_idx {frame_idx}")
                        status(f"SAM2 backward: {frame_idx} frames")
                log(f"SAM2 backward pass done — {backward_count} frames visited")
            else:
                log("SAM2 video: earliest click is at temp_idx 0 — backward pass skipped")

            # Translate temp_idx → range-relative index for the return dict.
            # Drop masks for frames OUTSIDE [in_f, out_f) — those are click-anchor
            # extras, not part of the user's render range.
            masks = {}
            for temp_idx, abs_f in enumerate(all_abs_frames):
                if not (in_f <= abs_f < out_f):
                    continue  # extra click frame — drop
                if temp_idx in masks_by_temp:
                    masks[abs_f - in_f] = masks_by_temp[temp_idx]
...
def show_preview_window(orig_bgr, keyed_rgb, alpha):
    ...
    # original.png is the RAW source frame (full green spill intact). The viewer's
    # "Original" view mode loads this so the user can compare the unprocessed
    # source against the keyed result. Without this file the viewer falls back to
    # fg.png (NN clean FG) for Original mode — which makes greens read as black
    # and is misleading. orig_bgr is already uint8 BGR — write as-is.
    ...
    try:
        ...
        if mt is not None and _s2_masks and frame_idx in _s2_masks:
            ...
            if _gate.shape != mt.shape:
                # 4K → 720p downsample. INTER_AREA (pixel area averaging) is
                # cv2's recommended method for shrinking — INTER_LINEAR was
                # producing visible staircase on diagonal silhouette edges
                # (especially thin features like legs).
                _gate = _cv2.resize(_gate, (mt.shape[1], mt.shape[0]),
                                    interpolation=_cv2.INTER_AREA)
                # Tiny Gaussian smooth (3px) erases any residual staircase
                # from the integer-grid resize and gives a sub-pixel edge.
                _gate = _cv2.GaussianBlur(_gate, (3, 3), 0)
            mt = mt * _gate
        if fg is not None and mt is not None:
            out_dir = ctx["scrub_dir"] / f"{frame_idx:03d}"
            out_dir.mkdir(parents=True, exist_ok=True)
    ...
    if settings.get("alpha_method") == 1:
        ...
        if _pos or _neg:
            ...
            try:
                ...
                if _n_good > 0:
                    # SCRUB uses single-anchor legacy behavior because sampled_tl_frames
                    # may be NON-CONTIGUOUS (subsampled when scrub range > Max Frames),
                    # so click frame is unlikely to land exactly on a sampled frame.
                    # Use the NEAREST sampled frame as the anchor — the actor barely
                    # moves between adjacent frames so this is essentially equivalent
                    # to placing clicks on the click frame itself.
                    # (Multi-frame click_groups infrastructure exists for PROCESS RANGE
                    # but SCRUB-with-extras would require pre-exporting non-sampled
                    # click frames to the temp dir — left as a future improvement. For
                    # now, multi-frame refining clicks are collapsed to a single anchor
                    # at the first click's frame; the rest are placed at that anchor's
                    # image coordinates which is already what _pos/_neg contain.)
                    ...
                    if _anch_abs is not None and sampled_tl_frames:
                        ...
                        log(f"SCRUB: click_frame={_anch_abs} nearest_sample={_good_tl[_anchor_rel]} "
                            f"(distance={_dists[_anchor_rel]} frames) → anchor_rel={_anchor_rel}")
                    else:
                        _anchor_rel = 0
            ...
            if settings.get("alpha_method") == 1:
                ...
                if (pos or neg) and braw_frames_dir:
                    try:
                        ...
                        # Build per-click groups in BRAW-relative indices (timeline_frame - in_f).
                        # Each click has its own frame_num so SAM2 places each click on the
                        # correct frame instead of forcing all clicks onto a single anchor.
                        # Skips clicks before in_f (out of BRAW export range).
                        _braw_click_groups = []
                        for _c in sam_clicks:
                            _cf = _c.get("frame")
                            if _cf is None:
                                continue
                            _rel = int(_cf) - in_f
                            if _rel < 0:
                                log(f"BRAW: skipping click frame {_cf} — before range start {in_f}")
                                continue
                            _braw_click_groups.append({
                                "x": int(_c["x"]), "y": int(_c["y"]),
                                "label": int(_c.get("label", 1)),
                                "frame": _rel,
                            })
                        ...
                        _braw_sam2_video_masks = run_sam2_video_propagation(
                            ...
                            click_groups=(_braw_click_groups if _braw_click_groups else None),
                        )
                        if _braw_sam2_video_masks:
                            log(f"SAM2 video propagation: {len(_braw_sam2_video_masks)} per-frame masks ready (multi_frame={bool(_braw_click_groups)})")
                        else:
        ...
        try:
            ...
            if (settings.get("alpha_method") == 1 and
                ...
                if braw_frames_dir:
                    _anchor_abs = sam_points.get("frame")
                    _anchor_in_tif = (_anchor_abs - in_f) if _anchor_abs is not None else None
                    # Per-click groups in BRAW-relative indices.
                    _br_click_groups = []
                    for _c in sam_clicks:
                        _cf = _c.get("frame")
                        if _cf is None:
                    ...
                    sam2_video_masks = run_sam2_video_propagation(
                        ...
                        _anchor_in_tif,
                        click_groups=(_br_click_groups if _br_click_groups else None),
                    )
                else:
                    # Per-click groups in absolute timeline frames (no offset needed
                    # for the regular VideoCapture path — function uses ss/cs/in_f
                    # internally to seek by absolute frame).
                    _reg_click_groups = [
                        {"x": int(c["x"]), "y": int(c["y"]),
                         "label": int(c.get("label", 1)),
                         "frame": int(c["frame"])}
                        for c in sam_clicks if c.get("frame") is not None
                    ]
                    sam2_video_masks = run_sam2_video_propagation(
                        fp, ss, cs, in_f, out_f,
                        sam_points.get("positive", []),
                        ...
                        click_groups=(_reg_click_groups if _reg_click_groups else None),
                    )
                if sam2_video_masks:
