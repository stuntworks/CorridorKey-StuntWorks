...
def generate_alpha_hint(frame, settings):
    # ALIGNED WITH AE 2026-04-26: switched from HSV-based AlphaHintGenerator
    # to AE's simple RGB chroma test. Reason: HSV thresholds [35-85, 50-255,
    # 50-255] mark green-spilled vest fabric as "screen color" — kicks vest
    # pixels out of the alpha hint, so the NN sees the vest as background and
    # produces gray inside it on the matte. AE never had this problem because
    # its RGB test requires green to dominate red AND blue by 20%, which only
    # genuinely-green pixels do — green spill on red/yellow/grey fabric still
    # has the original colour dominant.
