        ...
        try:
            ...
            for tf in range(in_f, out_f):
                ...
                if mt is not None and not bool(settings.get("sam2_bypass", False)):
                    ...
                    # 2026-05-14: gates split into two lists. _gates_for_sam carries
                    # the bypass-filtered set (drives Combined merge + viewer math).
                    # _gates_for_export ignores BYPASS MASK 1/2 so the SAM sidecar
                    # always contains every SAM object the user anchored — bypass is
                    # a viewer toggle, not an export filter. Caused the Both-mode
                    # silent-drop where checking BYPASS MASK 1 produced zero SAM files.
                    _gates_for_sam = []       # bypass-applied (Combined merge)
                    _gates_for_export = []    # bypass-ignored (SAM sidecar export)
                    _obj_ids = []
                    if sam2_video_masks and range_idx in sam2_video_masks:
                        ...
                        if isinstance(_per, dict):
                            ...
                            _items = [(1, _per)]
                        for _oid, _gm in _items:
                            _arr = np.asarray(_gm, dtype=np.float32)
                            _gates_for_export.append(_arr)
                            if _oid == 1 and bool(settings.get("mask1_bypass", False)):
                                continue
                            if _oid == 2 and bool(settings.get("mask2_bypass", False)):
                                continue
                            _gates_for_sam.append(_arr)
                            _obj_ids.append(_oid)
                    else:
                        ...
                        if _static_sam2_gate:
                            ...
                            for _oid, _g in _items:
                                _arr = np.asarray(_g, dtype=np.float32)
                                _gates_for_export.append(_arr)
                                if _oid == 1 and bool(settings.get("mask1_bypass", False)):
                                    continue
                                if _oid == 2 and bool(settings.get("mask2_bypass", False)):
                    ...
                    if _gates_for_sam:
                        _sam_union = union_binary_silhouettes(_gates_for_sam)
                    if _gates_for_export:
                        _sam_matte_v1 = process_sam_matte(
                            union_binary_silhouettes(_gates_for_export),
                            margin_px=float(settings.get("sam2_margin", 0)),
                            softness_sigma=float(settings.get("sam2_soften", 0)),
                            fill_kernel_px=int(settings.get("fill_holes", 0)),
                        )
