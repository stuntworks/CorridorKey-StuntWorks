...
def _panel_dispatch_sam2_combine(alpha, gates, src_rgb, settings, obj_ids=None):
    ...
    # Per-mask bypass: drop any mask flagged in settings before doing
    # anything else. Mirrors the viewer so render output matches preview.
    if obj_ids is not None:
        _m1b = bool(settings.get("mask1_bypass", False))
        _m2b = bool(settings.get("mask2_bypass", False))
        if _m1b or _m2b:
            _kept = [(g, oid) for g, oid in zip(gates, obj_ids)
                     if not (oid == 1 and _m1b)
                     and not (oid == 2 and _m2b)]
            if not _kept:
                return alpha
            gates = [g for g, _ in _kept]
            obj_ids = [oid for _, oid in _kept]
