    ...
    if _braw_tif_buffers:
        ...
        try:
            ...
            for fidx, _buf in enumerate(_braw_tif_buffers):
                ...
                # Pre-41a0a7f pattern: SAM merged INTO mt via _panel_dispatch_sam2_combine.
                # Surgical revert 2026-05-12 to test whether Phase B per-frame work is the
                # PROCESS RANGE deadlock source.
        ...
        try:
            ...
            for tf in range(in_f, out_f):
                ...
                if fg is not None and mt is not None:
                    ...
                    # Encode CK clip in memory — no file I/O on the worker thread.
                    # Codec selection picks bit depth + format.
                    if _codec == 3:
                        if _fmt == 0:
